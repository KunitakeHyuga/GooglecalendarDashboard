...
function DashboardApp() {
  ...
  async function loadOverall() {
    ...
    const data = await api(`/api/summary?timeMin=${encodeURIComponent(timeMin)}&timeMax=${encodeURIComponent(timeMax)}`);
    setOverallTotals(data.totals || []);
    setOverallGrandMinutes(data.grandTotalMinutes || 0);
  }

  async function loadStudy(calendarIdOverride) {
    const { timeMin, timeMax } = buildStudyRangeQuery();
    const { windowStart, windowEnd } = buildStudyRangeValues();
    const selectedId = calendarIdOverride || studyCalendarId;
    ...
    if (selectedId) {
      query.set('calendarId', selectedId);
    }

    const report = await api(`/api/study-report?${query.toString()}`);
    setStudyCalendarId(report.calendar?.id || '');
    setStats(report.stats || { todayMinutes: 0, monthMinutes: 0, totalMinutes: 0 });
    setTags(report.tags || []);
    setDailyStacked(report.dailyStacked || []);
    setRecent(report.recent || []);
  }

  async function loadSchedule(calendarIdOverride, weekStartOverride) {
    ...
    const data = await api(`/api/events-range?${query.toString()}`);
    setScheduleEvents(
      (data.events || []).map((item) => {
        const bg = item.color || '#3b82f6';
        return {
          id: item.id,
          title: item.title,
          start: item.start,
  ...
  async function initialize() {
    try {
      ...
      setUser(me.user);
      const calendarRes = await api('/api/calendars');
      const calendarItems = calendarRes.calendars || [];
      setCalendars(calendarItems);

      const initialStudyCalendarId = pickDefaultStudyCalendarId(calendarItems);
      setStudyCalendarId(initialStudyCalendarId);

      await Promise.all([loadOverall(), loadStudy(initialStudyCalendarId), loadSchedule()]);
    } catch (error) {
  ...
  useEffect(() => {
    if (!user || activeView !== 'study' || !studyCalendarId) return;
  ...
  useEffect(() => {
    if (!user || activeView !== 'schedule') return;
    ...
    try {
      if (scheduleEditor.mode === 'create') {
        await api('/api/events', {
          method: 'POST',
          body: JSON.stringify({
            calendarId: scheduleEditor.calendarId,
            summary,
            description: scheduleEditor.description,
            startDateTime: scheduleEditor.startDateTime,
            endDateTime: scheduleEditor.endDateTime,
        ...
        await api('/api/events', {
          ...
          body: JSON.stringify({
            calendarId: scheduleEditor.calendarId,
            eventId: scheduleEditor.eventId,
            summary,
            description: scheduleEditor.description,
            startDateTime: scheduleEditor.startDateTime,
            endDateTime: scheduleEditor.endDateTime,
        ...
        <div className="mx-auto w-[min(1180px,94vw)] py-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-400 md:text-sm">${user ? `${user.displayName} (${user.email || 'no email'})` : '読み込み中...'}</p>
            </div>
            <a className="rounded-md border border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-800" href="/auth/logout">ログアウト</a>
          </div>
